...
for batch_size in $(seq 0 1)
do
	for lr in $(seq 0 1)
	do
		for dropout in $(seq 0 1)
		do
			for num_neurons in $(seq 0 2)
			do
				folder=/workspace/results_keras/simple/hyper_param_xval2/${batch_size}${lr}${dropout}${num_neurons}/
				mkdir $folder
